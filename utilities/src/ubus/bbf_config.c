		    ...
		    struct blob_attr *msg)
{
	...
	// This is the only place in the bbfdm to apply dm-framework services before commit
	// dm-framework services doesn't apply in the handlers, it applies when the DM changes are ready to commit
	// changes ubus is supposed to be called by USP/CWMP applications before commit.
	trigger_dfm_service_transactions(ctx, "apply", tb[SERVICES_PROTO] ? blobmsg_get_string(tb[SERVICES_PROTO]) : "both");
